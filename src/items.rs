...
impl<P, R> RawComplexBackendItems<P, R> {
    ...
    pub fn to_non_raw(self) -> ComplexBackendItems<P, R> {
        ComplexBackendItems {
            packages: self.packages.into_iter().map(|x| (x.name.clone(), x)).collect(),
            repos: self.repos.into_iter().map(|x| (x.name.clone(), x)).collect(),
        }
    }
}
...
impl<P, R> ComplexBackendItems<P, R> {
    ...
    pub fn to_packages(self) -> BTreeMap<String, P> {
        self.packages
            .into_iter()
            .map(|(x, y)| (x.to_string(), y.options))
            .collect()
    }
    pub fn to_repos(self) -> BTreeMap<String, R> {
        self.repos
            .into_iter()
            .map(|(x, y)| (x.to_string(), y.options))
            .collect()
